...
class QrScannerComponent {
     ...
     * @return {?}
     */
    get findMediaDevices() {
        const /** @type {?} */ videoDevice = (dvc) => dvc.kind === 'videoinput' && dvc.label.search(/back/i) > -1;
        return new Promise((resolve, reject) => {
            if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
                try {
                    navigator.mediaDevices.enumerateDevices()
                        .then((devices) => {
                        const /** @type {?} */ device = devices.find((_device) => videoDevice(_device));
                        if (device) {
    ...
    load() {
        ...
        if (this.supported) {
            ...
            this.qrCode.myCallback = (decoded) => this.decodeCallback(decoded);
...
/** @nocollapse */
QrScannerModule.ctorParameters = () => [];
